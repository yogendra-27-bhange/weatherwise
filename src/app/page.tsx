...
export default function HomePage() {
    ...
    if (defaultLocationName) {
      searchLocation(defaultLocationName).then(locInfo => {
        ...
        toast({
            ...
            description: `Could not load default location: ${defaultLocationName}. Attempting geolocation.`,
            variant: "destructive",
        });
        attemptGeolocation();
      });
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          toast({
            ...
            description: "Could not auto-detect. Defaulting to London. You can set a default in Settings or search manually.",
            variant: "default", // Changed to default as it's an informative fallback
          });
          searchLocation("London").then(setCurrentLocation); // Fallback to London
        }
      );
    } else {
       toast({
        title: "Geolocation Not Supported",
        description: "Defaulting to London. You can set a default in Settings or search manually.",
      });
      searchLocation("London").then(setCurrentLocation); // Fallback to London
    }
  };
  ...
  const handleSearch = async (e?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (locInfo.name.toLowerCase() === 'unknown city' || locInfo.name.toLowerCase() === 'search error') {
        toast({
  ...
  useEffect(() => {
    if (currentLocation && currentLocation.name) {
      setIsLoading(true);
      setWeatherData(null); // Clear previous data while new data is loading
      getWeatherData(currentLocation)
        .then(data => {
          setWeatherData(data);
          if (!isLoading) { // Only toast on subsequent updates, not initial load
            toast({
    ...
    } else if (!currentLocation && !isSearching){ // If no location info yet and not actively searching, stop loading
        setIsLoading(false);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentLocation]);
  ...
  const getPageBackgroundClass = () => {
    if (!weatherData || !weatherData.current) return 'bg-gradient-to-br from-sky-400 to-blue-600'; // Default background
    const code = weatherData.current.conditionCode;
    const isDay = weatherData.current.isDay;

    if (code.startsWith('01')) return isDay ? 'bg-gradient-to-br from-yellow-300 via-orange-400 to-red-500' : 'bg-gradient-to-br from-indigo-700 via-purple-800 to-slate-900'; // Clear
    ...
    if (code.startsWith('02') || code.startsWith('03') || code.startsWith('04')) return isDay ? 'bg-gradient-to-br from-sky-300 via-cyan-400 to-teal-500' : 'bg-gradient-to-br from-slate-600 via-gray-700 to-slate-800'; // Clouds (day/night)
    ...
    <div className={`flex flex-col items-center min-h-screen p-4 md:p-8 transition-colors duration-500 ${getPageBackgroundClass()}`}>
        ...
        <Card className="shadow-xl bg-background/80 backdrop-blur-md">
          <CardContent className="p-4 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <form onSubmit={handleSearch} className="flex w-full md:w-auto gap-2">
              ...
              />
              <Button type="submit" disabled={isSearching || !searchQuery.trim()} className="bg-accent hover:bg-accent/90">
      ...
      <footer className="w-full max-w-7xl mt-12 text-center">
        <p className="text-sm text-background/80">
          WeatherWise &copy; {new Date().getFullYear()}. Weather data by OpenWeatherMap. News by NewsAPI.org. Videos by YouTube.
        </p>
        <p className="text-sm text-background/80 mt-2">
          Developed by Yogendra Bhange ,Yash Dudhe ,Prathmesh Saharkar & Priyanshu Pethe
        </p>
